...
- **`lib/stores/video-store.ts`** — playback state. **Not** persisted. Has a `source: "local"|"youtube"` discriminator. Holds a `videoRef` typed as `RefObject<MediaController|null>|null` where `MediaController` is the minimal interface `{play():Promise<void>; pause():void; currentTime; readonly duration; volume}` — registered by both `LocalVideoSurface` (the `<video>` element) and `YouTubeSurface` (react-player's ref); for YouTube, playback is driven by the `playing`/`volume` props rather than imperative play/pause calls on the controller. `setVideoUrl(url, source?="local")` switches source. `setVolume` writes both store state and the active controller's `volume`. `clearVideoRef()` and `clearVideo()` handle teardown. `togglePlayPause` is source-aware: imperative DOM call for local, controlled-prop flip for YouTube. Keep this two-way coupling in mind — changing playback state usually means also calling the DOM element (local) or flipping a prop (YouTube).
...
`VideoArea` (inside `VideoPlayer`) mounts exactly one of three surfaces based on store state: `LocalVideoSurface` (existing `<video>` element + `useVideoPlayer` RAF-based time tracking, frame-accurate), `YouTubeSurface` (react-player v3 + `useYouTubePlayer`, not frame-accurate — bound by YouTube's API time granularity — but smooth: youtube-video-element's `currentTime` getter is live (it calls YT `getCurrentTime()` on every read), so `useYouTubePlayer` runs a RAF loop driven by `onPlay`/`onPause` that tracks it for per-frame playhead motion. The bare `onTimeUpdate` event alone fires only ~100–250ms apart, which made the playhead step. The loop keeps a **monotonic** displayed time: it follows the live time when that's ahead (steady playback ≈ plain polling), and glides forward just under 1x when the live time *stalls* — which it does for ~200–300ms right after resume before jumping to catch up (reading the raw value there shows a freeze-then-teleport). Displayed time never moves backward except on a real seek (`onSeeked`/external), so the playhead can't jitter. Do **not** push the synthetic/leading time back via the `currentTime` *setter* — that triggers a real `seekTo` and hitches playback; the `SELF_WRITE_EPSILON` guard in the external-seek effect keeps the loop's own writes from doing that), or `VideoUploader` (no video loaded yet). On-screen controls (play/pause, seek, volume, mute) route through store actions regardless of source. Both surfaces render the current subtitle as an **absolute overlay** at the bottom of the player (identical placement/style for visual consistency); on the YouTube embed the overlay is `pointer-events-none` so the player stays clickable underneath (note: overlaying the embed is a deliberate UX choice that trades against YouTube's "don't obscure the player" guidance). `VideoUploader` is the empty-state picker, a left/right split — **local file** (drag & drop / click) on one side, **YouTube link** on the other; the URL is validated by `isYouTubeUrl` in `components/editor/utils/videoUtils.ts` before calling `setVideoUrl(url,"youtube")`. The NavBar's `ToolBar` "Select Video" button calls `clearVideo()` to return to this picker (the single place a source is chosen). react-player is the only new runtime dependency added for YouTube support.
