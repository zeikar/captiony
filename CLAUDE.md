...
`VideoArea` (inside `VideoPlayer`) mounts exactly one of three surfaces based on store state: `LocalVideoSurface` (existing `<video>` element + `useVideoPlayer` RAF-based time tracking, frame-accurate), `YouTubeSurface` (react-player v3 + `useYouTubePlayer`, ~100ms time resolution via `onTimeUpdate` — not frame-accurate), or `VideoUploader` (no video loaded yet). On-screen controls (play/pause, seek, volume, mute) route through store actions regardless of source. For the YouTube branch, subtitle text renders **outside** the embed — in a strip below the player (a flex sibling, not an absolute overlay on the iframe) — to comply with YouTube ToS (the local branch keeps its absolute over-video overlay). `VideoUploader` accepts both a local file and a YouTube URL; the URL is validated by `isYouTubeUrl` in `components/editor/utils/videoUtils.ts` before calling `setVideoUrl(url,"youtube")`. react-player is the only new runtime dependency added for YouTube support.
